...
function FindPath()
{
    ...
    //matrix.printGraph();
    let results = matrix.dijkstraAlgorithm("START");
...
function FormatFile()
{
    let allText = fs.readFileSync("Additional-Files\\p082_matrix.txt", 'utf8');
...
class weightedMatrix {
    ...
    dijkstraAlgorithm(startingNode)
    {
        ...
        {
            visited[key] = {dist: 999999999999, path:undefined};
        }
    ...
    _generateEdges(tree, length)
    {
        for (let i = 0; i < length; ++i)
        {
            this.newEdge("START", (i).toString() + ",0", tree[i][0]);
        }
        ...
        for (let i = 0; i < length - 1; i++)
        {
            ...
            {
                this.newEdge(i.toString() + "," + j.toString(), (i).toString() + "," + (j+1).toString(), tree[i][j+1]);
                this.newEdge(i.toString() + "," + j.toString(), (i + 1).toString() + "," + (j).toString(), tree[i+1][j]);
            }
        }
        ...
        for (let j = 0; j < length - 1; j++)
        {
            this.newEdge((length - 1).toString() + "," + j.toString(), (length - 1).toString() + "," + (j+1).toString(), tree[length - 1][j+1]);
            this.newEdge(j.toString() + "," + (length - 1).toString(), "FINAL", 0);
        }

        this.newEdge((length - 1).toString() + "," + (length - 1).toString(), "FINAL", 0);
