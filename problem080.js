...
function ComputeTotalDecimalSum()
{
	let arr = [];
	for (let i = 2; i <= 100; ++i)
	{

		let answer = computeSquareRoot(i, 99);
		if (answer != 0)
		{
			let index = answer.indexOf(".");
			answer = answer.substring(0, index) + answer.substring(index + 1);
			answer =  answer.split("").reduce((a,b) => {return Number.parseInt(a) + Number.parseInt(b);});			
			arr.push(answer);
		}
...
function computeSquareRoot(number, precision)
{
    ...
    while (start <= end)
    {
        ...
        {
            start = mid + 1;
            ans = new BigNumber(mid.toString());
        }
        else
        {
            end = mid - 1;
    ...
    let increment = new BigNumber("0.1");

    for (let i = 0; i < precision; i++)
    {
        while(ans.multiply(ans).compare(number) < 0)
        {
			ans = ans.add(increment);
        }

        ans = ans.subtract(increment);
        increment = increment.divide(10);
    }
